...
If recovery must remove a malformed or mismatched marker, it first binds that result ID to its original digest in implementation-owned round storage. A copy in the review bundle gives `review-submit` early feedback, but implementation-side status and application use only the authoritative binding; a missing, malformed, or mismatched advisory copy cannot veto it. Corrected content must use a new result ID.
