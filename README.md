...
Start a run with an approved UTF-8 Markdown task. The Implementer, Reviewer, and base options default to `.agent-squad/config.json`; pass them explicitly to override those selections for this run. Repeat `--context` to capture only the additional files the run needs.
