...
from tests._support import add_src_to_path, initialize_git_repository, run, run_cli
...
class InitCommandTests(unittest.TestCase):
    ...
    def test_init_creates_configuration_and_local_exclusions_only(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            temporary_root = Path(temporary_directory)
            ...
            exclude_path.write_bytes(b"# existing local rules\nprivate-output/\n")
    ...
    def test_repeated_init_does_not_rewrite_configuration_or_exclusions(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            temporary_root = Path(temporary_directory)
            repository = temporary_root / "repository"
            initialize_git_repository(repository)
            ...
            self.assertEqual(original_exclude.count(b".agent-squad-review/\n"), 1)
    ...
    def test_existing_valid_configuration_is_not_overwritten(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            custom_bytes = json.dumps(configuration, separators=(",", ":")).encode()
    ...
    def test_invalid_existing_configuration_causes_no_other_changes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            temporary_root = Path(temporary_directory)
            repository = temporary_root / "repository"
            initialize_git_repository(repository)
    ...
    def test_non_git_directory_is_rejected_without_partial_initialization(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            temporary_root = Path(temporary_directory)
    ...
    def test_conflicting_control_root_is_rejected_before_exclude_changes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            temporary_root = Path(temporary_directory)
            repository = temporary_root / "repository"
            initialize_git_repository(repository)
    ...
    def test_init_from_linked_worktree_updates_common_git_exclude(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            temporary_root = Path(temporary_directory)
            primary = temporary_root / "primary"
            linked = temporary_root / "linked"
            initialize_git_repository(primary)
            run(["git", "config", "user.name", "Agent Squad Tests"], cwd=primary)
            run(
            ...
            self.assertIn(".agent-squad/\n", exclude)
            self.assertIn(".agent-squad-review/\n", exclude)
            self.assertEqual(run(["git", "status", "--short"], cwd=linked).stdout, "")
