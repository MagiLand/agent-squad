...
| Fewer Developer turns per issue | For every issue the Developer approves a Task drafted from an issue that is already fully specified. After every approval the Developer gives the merge instruction, then fast-forwards the primary checkout or asks the Implementer to, because `pr merge` only prints the command (§7.10 step 5). None of these steps is a decision for a routine issue |

Non-goals, unchanged from §19 unless listed here: CI result enforcement or interpretation by the tool, the `rebase` merge method, automatic garbage collection of Herdr or Git resources, stacked PRs, Windows, any harness other than Claude Code and Codex, GitHub Enterprise hosts, and Forgejo instances older than 16.0.0.
...
- `pr merge` runs only on the Developer's instruction, given at the time or in advance (decisions 31 and 33), and requires `approved`, which now includes (5a) and (5b). Under a standing instruction the next action becomes `merge` only after both approvals. A forge refusal is reported as in v0.5.0.
...
- `pr create`, `status`, `review post`, `pr merge`, and skills (#74, decisions 31 and 33):
  - Without `--task`, `pr create` copies a ready issue into `## Task`, and the Implementer starts without presenting a Task.
  - Right after `pr create`, the Implementer records a standing merge instruction: a general `DECISION` whose body opens with `Standing merge instruction: merge when approved.`. A body that opens with `Standing merge instruction withdrawn.` withdraws it. While the instruction is in force, the next action `approved` becomes `merge`, and `status --json` names the decision as `merge_instruction`.
  - A verbatim review-before-merge rule in both skills says which PRs wait for the Developer's review. The Reviewer marks such a PR with a `## Merge hold` section after `## Findings`: `status` then keeps `approved` and reports `merge_hold`, and `pr merge` refuses with exit 4, naming the review, unless it is given `--accept-merge-hold`.
  - Before an automatic merge, the Implementer checks CI at the approved head. After a moved base, it brings the branch up to date and has it reviewed again. After the merge, it waits for the push run and sends one report.
...
Release target Friday 2026-10-02; hard date Monday 2026-10-05. Seven chores run first and independently, except that C7 follows C6; C3 runs before the next loop PR merges. The rest form a linear chain as in v0.5.0. Each is one issue in milestone `v0.6.0`.
...
| C7 | Chore: a started issue goes through to the merge without routine confirmations; a ready issue is the Task; the standing merge instruction is recorded by default; high-risk PRs wait for the Developer's review before they merge (#74) | 09-24 |
...
| 2 | Single-identity mode on GitHub: configuration, approval validity, `status`, `pr merge`, skills; `await_human_approval` placed before the `merge` that the standing instruction of C7 produces; trial 2 | 09-28 to 09-29 |
...
Development runs through the v0.5.0 loop itself: `squad-implementer` and a fresh `squad-reviewer` per pass, with the installed runtime and skills upgraded only between PRs from merged `main`, so no PR is reviewed by its own unmerged code. The Developer plans, triages, and gives a second opinion on `needs_human`. The Developer reviews routine PRs after they merge and held PRs before (decision 33).

## 10. Decisions record (2026-09-21 to 2026-09-23)
...
27. (2026-09-22) On an optional thread `rejected` names its reason, `Not pursued:` or `Deferred to #<open issue>:`, and the tool checks the issue; no new keywords, protocol tag unchanged. The "approved, ready to merge" report lists every optional finding with its disposition so the Developer decides with the findings in view; a fix after approval moves the head and needs another pass. Decision 33 moves this report after the merge for routine PRs.
...
31. (2026-09-22) The merge stays gated on the Developer's instruction, and no third skill takes over the steps after approval: the Implementer already holds the PR, the worktree ownership, and the primary checkout. The Developer may give the instruction in advance, for example "start on #N and merge when approved". The Implementer then records it as a general `DECISION` whose body opens with a fixed line that the delta defines, so the instruction lives on the PR, is attributable, and survives an interrupted session; the Developer may also post it from a `developer_accounts` login. While it is in force, `status` reports the next action `merge` instead of `approved`; the Implementer runs `pr merge` without waiting and then reports the merge, the cleanup, and every optional finding with its disposition. It never extends to a moved base, a forge refusal, or incomplete cleanup; the Implementer reports those and waits, as today. A newer `STOPPED` or Task amendment cancels it, and a later general decision that says so and links it withdraws it (§7.6); it neither lifts a stop nor settles `needs_decision`. In `single` mode the Implementer still learns of the human approval only through "Check PR #N" (decision 22). Waiting stays the default. There is no configuration switch, because a merge instruction belongs on the PR it authorises, and the forge's own auto-merge is not used, because it checks branch protection rather than §7.10 and does no cleanup. No tagged-line grammar change. The delta (#55) specifies it; Increment 2 (#57) implements it beside `await_human_approval`. Decision 33 amends this decision.
32. (2026-09-22) After a verified merge, `pr merge` fast-forwards the primary checkout to the verified base tip when that checkout has the base branch checked out and no changes to tracked files; otherwise it reports why and, when the checkout is on the base branch, prints the command, and the Implementer never runs the command itself. A fast-forward cannot lose work, and Git refuses when a local change or an untracked file would be overwritten, so the reason behind "never modifies the base-branch checkout" (§7.10 step 5) does not apply to it. Together with #66, the Developer's "merge" becomes the only step after approval. Filed as chore #69. Decision 33 removes that step for routine PRs.
33. (2026-09-23) The Developer says which issue to work on, and the Implementer carries it through to the merge. It asks only when a confirmation or a decision is genuinely needed. The Developer accepts reviewing routine work after it merges. A PR with high risk, security concerns, or another genuine need for human involvement waits for the Developer's review before it merges.
    - **The issue is the Task.** When the started issue is ready, `pr create` without `--task` copies it into `## Task` without rewording, and the Implementer starts without presenting a Task. Ready means that the issue is open, states what to build and its acceptance criteria, carries none of the triage labels `needs-triage`, `needs-info`, `ready-for-human`, or `wontfix`, and has not been changed by a later comment from the Developer. The copy must be exact because the Reviewer judges against the Task, not the issue. `pr create` refuses a closed issue, a pull request, and an empty body. The Implementer drafts a Task and asks only when the issue is not ready, or when the Developer's start instruction changes the scope or asks to see the Task.
    - **Starting an issue authorises the merge.** Right after `pr create`, the Implementer records the standing instruction of decision 31, with the Developer's start instruction quoted. It does not when the Developer keeps the merge for themselves or a merge hold applies. Waiting is therefore no longer the default.
      - The instruction opens with `Standing merge instruction: merge when approved.`, and a withdrawal opens with `Standing merge instruction withdrawn.`. A decision that opens with either line carries nothing else: no `## Task` and no `budget=`. `status` reports any other such decision as malformed and ignores it.
      - Neither line lifts a stop (§7.7) or settles `needs_decision`, and the Reviewer does not treat either as a design decision.
      - After it records a continuation (§7.7) or a Task amendment, the Implementer records the instruction again unless the Developer said otherwise.
      - `status --json` gains `merge_instruction` (the decision's ID, author, and time, or `null`) and `merge_hold`.
      - The Herdr sentence for `approved` (§8.5) stays as it is, because a standing instruction is the Developer's instruction.
    - **Review before merge.** A verbatim rule, whose text #74 item 3 fixes, goes into §12.2, §12.3, and both skills. A PR waits for the Developer's review when a defect in it could cause harm that reverting the PR would not undo, or would weaken the checks that later PRs rely on. The rule's six items name such changes; among them are security-sensitive code, irreversible changes to data or history, changes to what permits a review, an approval, a decision, or a merge or to what an agent may do without asking, new third-party dependencies and CI actions or changes to CI permissions, secrets, or triggers, effects outside the repository, and product, design, or scope questions left to the Developer or work beyond the Task. The items describe the PR's own changes, not the loop's routine steps, and the Task can also require the review.
      - The Implementer applies the rule at `pr create` and after every push. It holds a PR by not recording the instruction, or by withdrawing it, and names the item and the reason under "Areas worth extra review".
      - The Reviewer applies the rule to the whole PR on every pass. It holds a PR with a `## Merge hold` section after `## Findings`, whose first line is `Item <n>: <reason>` or `Task: <reason>`. The hold never changes the verdict.
      - `status` reports the hold in the latest tagged review as `merge_hold` and keeps the next action `approved`. `pr merge` refuses with exit 4 while the latest tagged review carries a hold, naming the review, unless it is given `--accept-merge-hold`.
      - Only the Developer releases a hold, by instructing the merge after seeing it. Only then does the Implementer pass `--accept-merge-hold`.
    - **CI and the reports.**
      - On `merge`, before it runs `pr merge`, the Implementer confirms that every check run for the approved head concluded `success`, `neutral`, or `skipped`. It waits for running checks with `gh run watch <run-id> --exit-status`, which joins the skill's read-only allowance. A failed check is a defect: the Implementer fixes it, or reports it when the fix is outside the Task. When no check exists for the approved head although the repository runs checks on pull requests, it reports that and waits. The tool itself still does not read CI (the non-goal in section 1 of this plan).
      - After the merge, the Implementer waits for the push run at the merge commit and sends one report that needs no answer. If that run failed, the report asks the Developer to choose between a fix and a revert.
      - When the next action stays `approved`, because no instruction is in force or a hold applies, the Implementer sends today's ready-to-merge report with the hold's item and reason, and waits.
    - **Moved base.** A moved base no longer stops the loop by itself. The Implementer merges the base branch into the PR branch (a merge, not a rebase), resolves any conflicts within the Task, validates, pushes, and has the new head reviewed. It reports and waits when the review budget is used up or a conflict needs a choice outside the Task. A review of the same head would not help, because it keeps the old merge-base. `--accept-moved-base` stays the Developer's choice.

    This decision replaces four parts of decision 31: that the instruction is recorded only when the Developer gives it ("Waiting stays the default"); its exclusion of a moved base; the withdrawal, which is now a decision opening with the fixed withdrawal line rather than one that links the instruction; and its delivery: #74 fixes both lines and implements them, where decision 31 left the line to the delta (#55) and the implementation to Increment 2 (#57). The rest of decision 31 stands, including no configuration switch and no forge auto-merge. It also moves the report of decision 27 after the merge for routine PRs and removes the step that decision 32 left after approval. As decision 29 records for #64 and #65, #74 amends `docs/agent-squad-v0.5.0-spec.md` in place with an entry in the Amendments list, and the delta takes the amended text as its baseline. Because #74 changes what permits a merge, its Reviewer runs the Reviewer skill from `main`, and the changed skills are installed only after it merges. Filed as chore #74, which follows #69 because both change section 8 of the Implementer skill.
