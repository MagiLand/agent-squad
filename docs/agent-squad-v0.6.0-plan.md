...
- `pr create`, `status`, `review post`, `pr merge`, and skills (#74, decisions 31 and 33):
  ...
  - Right after `pr create`, the Implementer records a standing merge instruction: a general `DECISION` whose body opens with `Standing merge instruction: merge when approved.`. A body that opens with `Standing merge instruction withdrawn.` withdraws it. While the instruction is in force, the next action `approved` becomes `merge`, and `status` names the decision.
  - A verbatim review-before-merge rule in both skills says which PRs wait for the Developer's review. The Reviewer marks such a PR with a `## Merge hold` section: `status` then keeps `approved`, and `pr merge` refuses without `--accept-merge-hold`.
...
33. (2026-09-23) The Developer says which issue to work on, and the Implementer carries it through to the merge. It asks only when a confirmation or a decision is genuinely needed. The Developer accepts reviewing routine work after it merges. A PR with high risk, security concerns, or another genuine need for human involvement waits for the Developer's review before it merges.
    - **The issue is the Task.** When the started issue is ready, `pr create` without `--task` copies it into `## Task` without rewording, and the Implementer starts without presenting a Task. Ready means that the issue is open, states its acceptance criteria, carries no triage label saying it is not ready, and has not been changed by a later comment from the Developer. The copy must be exact because the Reviewer judges against the Task, not the issue. The Implementer drafts a Task and asks only when the issue is not ready, or when the Developer's start instruction changes the scope or asks to see the Task.
    - **Starting an issue authorises the merge.** Right after `pr create`, the Implementer records the standing instruction of decision 31. It does not when the Developer keeps the merge for themselves or a merge hold applies. Waiting is therefore no longer the default.
    - **Review before merge.** A verbatim rule in both skills lists what holds a PR: security, irreversible effects, changes to what permits a review or a merge, a new third-party dependency or a change to CI permissions, external or incompatible effects, and open product or scope questions. The Task can also ask for it.
      - The Implementer holds a PR by not recording the instruction, or by withdrawing it.
      - The Reviewer holds a PR with a `## Merge hold` section in its review, which `status` and `pr merge` honour.
      - Only the Developer releases a hold. Only then does the Implementer pass `--accept-merge-hold`.
    - **CI and the final report.** Before an automatic merge, the Implementer confirms that CI passed at the approved head; the tool still does not read CI (§1). After the merge, it waits for the push run and sends one report that needs no answer.
    - **Moved base.** A moved base no longer stops the loop. The Implementer merges the base branch into the PR branch, validates, pushes, and has the new head reviewed. A review of the same head would not help, because it keeps the old merge-base. `--accept-moved-base` stays the Developer's choice.

    This decision replaces decision 31's "Waiting stays the default" and its exclusion of a moved base. The rest of decision 31 stands, including no configuration switch and no forge auto-merge. Filed as chore C7 (#74), which follows #69 because both change section 8 of the Implementer skill. It moves decision 31's `status` change forward from Increment 2 (#57).
