...
33. (2026-09-23) The Developer says which issue to work on, and the Implementer carries it through to the merge. It asks only when a confirmation or a decision is genuinely needed. The Developer accepts reviewing routine work after it merges. A PR with high risk, security concerns, or another genuine need for human involvement waits for the Developer's review before it merges.
    ...
    - **Review before merge.** A verbatim rule, whose text #74 item 3 fixes, goes into §12.2, §12.3, and both skills. A PR waits for the Developer's review when a defect in it could cause harm that reverting the PR would not undo, or would weaken the checks that later PRs rely on. The rule's six items name such changes; among them are security-sensitive code, irreversible changes to data or history, changes to what permits a review, an approval, a decision, or a merge or to what an agent may do without asking, new third-party dependencies and CI actions, effects outside the repository, and product, design, or scope questions left to the Developer. The items describe the PR's own changes, not the loop's routine steps, and the Task can also require the review.
