...
v0.6.0 delivers the two items §19 of the v0.5.0 delta deferred, plus five small operational items the v0.5.0 release trials and the first loop PR exposed:
...
- `pr merge` runs only on the Developer's instruction and requires `approved`, which now includes (5a) and (5b). A forge refusal is reported as in v0.5.0.
...
Release target Friday 2026-10-02; hard date Monday 2026-10-05. Five chores run first and independently, C3 before the next loop PR merges; the rest form a linear chain as in v0.5.0. Each is one issue in milestone `v0.6.0`.
...
| 0b | The v0.6.0 specification delta over v0.5.0 (§4.5, §7.3, §7.5, §7.9, §7.10, §8.3 if C2 warrants, §9, §10, §11, §12, §16, §17, §18, §19), written after 0a so it contains no unverified claim | 09-24 to 09-26 |
| 1 | The `Forge` protocol, neutral types, factory; GitHub behind it unchanged; runtime upgrade and trial 1 | 09-26 to 09-27 |
| 2 | Single-identity mode on GitHub: configuration, approval validity, `status`, `pr merge`, skills; trial 2 | 09-28 to 09-29 |
